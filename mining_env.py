...
class MiningEnv(gym.Env):
    ...
    def __init__(self, board_shape=(10, 10), max_energy=100, minable_path="minable.json", minor_rewards=0.0):
        ...
        self.observation_space = spaces.Dict({
            "dust": spaces.Box(0, 6, shape=board_shape, dtype=np.int8),
    ...
    def reset(self, seed=None, options=None):
        ...
        self.reward_masks = []
        self.reward_values = []
        self.retrieved = np.zeros(4, dtype=np.int8)
        tries = 0
        while not self._place_rewards():
            tries += 1
            if tries > 1000:
                raise RuntimeError("Failed to place rewards after many attempts")
        self._place_rewards()
        self._spread_dust()
        obs = self._get_obs()
        return obs, {}
    ...
    def _spread_dust(self):
        ...
        # Start with a random seed point, even dust layer
        x, y = random.randint(0, self.board_shape[0]-1), random.randint(0, self.board_shape[1]-1)
        dust[x, y] = random.choice([l for l in range(min_layer, max_layer+1) if l % 2 == 0])
        queue = [(x, y)]
        visited = set(queue)
        while queue:
            cx, cy = queue.pop(0)
            for dx, dy in [(-1,0),(1,0),(0,-1),(0,1)]:
                ...
                if 0 <= nx < self.board_shape[0] and 0 <= ny < self.board_shape[1]:
                    if (nx, ny) not in visited:
                        # Set dust layer to be within 2 of neighbors, and even
                        ...
                        min_n = max(min(neighbor_layers)-2, min_layer)
                        max_n = min(max(neighbor_layers)+2, max_layer)
                        # Only even layers
                        even_layers = [l for l in range(min_n, max_n+1) if l % 2 == 0]
                        if even_layers:
                            dust[nx, ny] = random.choice(even_layers)
                        else:
                            # fallback: pick closest even in [min_layer, max_layer]
                            dust[nx, ny] = min_layer if min_n < min_layer else max_layer
                        queue.append((nx, ny))
                        visited.add((nx, ny))
        self.dust = dust
    ...
    def step(self, action):
        ...
        # Store uncovered counts BEFORE applying dust changes (for minor rewards)
        prev_uncovered = []
        if self.minor_rewards > 0:
            for i, mask in enumerate(self.reward_masks):
                if not self.retrieved[i]:
                    prev_uncovered.append(np.sum(self.dust[mask] == 0))
                else:
                    prev_uncovered.append(0)
        ...
        if tool == 0:
            ...
            # Blower: 2 on center and 4-neighbors, 1 on outer 4 diagonal grids
            self.energy -= 4
            self.dust[x, y] = max(0, self.dust[x, y] - 2)
            for dx, dy in [(-1,0),(1,0),(0,-1),(0,1)]:
                nx, ny = x+dx, y+dy
        ...
        for i, mask in enumerate(self.reward_masks):
            ...
            if self.minor_rewards > 0:
                ...
                newly_uncovered = now_uncovered - prev_uncovered[i]
